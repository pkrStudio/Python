...
score_number_font=pygame.font.Font(None,28)
game_over_font=pygame.font.Font(None,48)
play_again_font=pygame.font.Font(None,28)
score_msg=score_font.render("Score : ",1,pygame.color("green"))
score_msg_size=score_font.size("Score")
background_color=pygame.Color(0,0,0)
black=pygame.Colo(0,0,0)


gameClock=pygame.time.Clock()
def checkCollision(posA,As ,posB, Bs):
    if(posA.x < posB.x+Bs and posB.x and posA.y <posB.y+Bs and posA.y+As > posB.y):
        return True
    return False
...
class snake:
    ...
    #make the function of crashing when snake eats itself
    ...
    def draw(self,screen):
        ...
        while(counter<len(self.stack)):
            if(self.stak[counter].color=="NULL"):
                counter +=1
                continue
...
def endGame():
    ...
    sys.exit(0)
def drawScore(score):
    score_numb=score_number_font .render(str(score),1,pygame.Color("red"))
    screen.blit(score_msg,(SCREEN_WIDTH - score_msg_size[0]-60,10))
    screen.blit(score_numb,(SCREEN_WIDTH -45,14))
    
def drawGameTime(gameTime):
    game_time =score_font.rander("time:",1,pygame.Color("white"))
    game_time_numb= score_number_font.render(str(gameTime/1000),1,pygame.Color("white"))
...
def respawnApple(apples,index,sx,sy):
    ...
    while(angle >radius):
        ...
        if(x==sx and y==sy):
            continue
    newApple =apples(x,y,1)
    apples[index]=newApple
    
def respawnApples(apples,quantity,sx,sy):
    counter=0
    del apples[:]
    radious=math.sqrt((SCREEN_WIDTH/2*SCREEN_WIDTH/2+ SCREEN_HEIGHT/2*SCREEN_HEIGHT/2))/2
    angle=999
    while(counter < quantity):
        while(angle>radious):
            angle=random.uniform(0, 800)*math.pi*2
            x=SCREEN_WIDTH/2 +radious*math.cos(angle)
            y=SCREEN_HEIGHT/2 + radious* math.sin(angle)
            if(x-APPLE_SIZE==sx or x+APPLE_SIZE==sx) and( y-APPLE_SIZE==sy or y+APPLE_SIZE==sy or radious-angle<=0):
                continue
        apples.append(apples(x,y,1))
        angle=999
        counter +=1
def main():
    score=0
...
# initialization of snake
  ...
  #  start_segments-=1
    ...
    max_apples=1
    eatne_apple=False
    apples=[max_apples(random.randint(60, SCREEN_WIDTH),random.randint(60, SCREEN_WIDTH),1)]
    respawnApple(apples,max_apples,mysnake.x,mysnake.y)
                
    startTime=pygame.time.get_tickets()
    endgame=0
    
    while(endgame !=1):
        gameClock.ticket(FPS)
        ...
        mysnake.draw(screen)
        drawScore(score)
        gameTime=pygame.time.get_tickets()-startTime
